...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnOpusPacketEncoded, const TArray<uint8>&, Packet);
/**
...
public:
	...
	UPROPERTY(BlueprintAssignable, Category = "Runtime Audio Importer|Delegates")
	FOnOpusPacketEncoded OnOpusPacketEncoded;
	/**
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Runtime Audio Importer|Convert", meta=(DeprecatedFunction, DeprecationMessage = "This function is deprecated due to reliability issues. It may work correctly in the editor but not in packaged projects (or vice-versa) and behavior varies between UE versions. Instead, import audio files with the .imp extension to create Imported Sound Wave assets directly in the editor, then use 'Import Audio From Pre-Imported Sound'. See: https://docs.georgy.dev/runtime-audio-importer/pre-import-audio"))
