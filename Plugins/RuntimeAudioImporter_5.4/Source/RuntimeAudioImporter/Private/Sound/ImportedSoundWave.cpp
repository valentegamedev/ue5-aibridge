...
int32 UImportedSoundWave::OnGeneratePCMAudio(TArray<uint8>& OutAudio, int32 NumSamples)
{
	...
	TArray<float> PCMDataForCallback;
...

	{
		...
		// Copy audio data for output
		AudioDataCopy.SetNumUninitialized(RetrievedPCMDataSize);
		FMemory::Memcpy(AudioDataCopy.GetData(), RetrievedPCMDataPtr, RetrievedPCMDataSize);

		// Copy PCM data for potential callback (do this while we still have access to the data)
		PCMDataForCallback.SetNumUninitialized(NumSamples);
		FMemory::Memcpy(PCMDataForCallback.GetData(), RetrievedPCMDataPtr, NumSamples * sizeof(float));
	...
	{
		FRAIScopeLock CallbackLock(&OnGeneratePCMData_DataGuard);
		ShouldTriggerCallback = OnGeneratePCMDataNative.IsBound() || OnGeneratePCMData.IsBound();
	}

	OutAudio = MoveTemp(AudioDataCopy);

	if (ShouldTriggerCallback)
	{
		AsyncTask(ENamedThreads::GameThread, [WeakThis = MakeWeakObjectPtr(this), PCMData = MoveTemp(PCMDataForCallback)]() mutable
		{
			if (WeakThis.IsValid())
			{
...
void UImportedSoundWave::SetNumSamplesPerChunk(int32 NumSamples)
{
	...
	else if (!DesiredNumSamplesPerChunk.IsSet() || NumSamples != DesiredNumSamplesPerChunk.GetValue())
	{
