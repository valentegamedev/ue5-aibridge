...
{
	VADInstance = bVAD ? URuntimeVoiceActivityDetector::CreateRuntimeVoiceActivityDetector() : nullptr;
	if (VADInstance)
	{
...
bool UStreamingSoundWave::SetVADProvider(TSubclassOf<URuntimeVADProviderBase> Provider)
{
	if (VADInstance)
	{
		return VADInstance->SetVADProvider(Provider);
	}
	UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Unable to set VAD provider as the VAD instance is not valid"));
	return false;
}

URuntimeVADProviderBase* UStreamingSoundWave::GetVADProvider() const
{
	if (VADInstance)
	{
		return VADInstance->GetVADProvider();
	}
	UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Unable to get VAD provider as the VAD instance is not valid"));
	return nullptr;
}
