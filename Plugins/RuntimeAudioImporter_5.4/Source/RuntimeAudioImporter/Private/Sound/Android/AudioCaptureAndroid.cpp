...
Java_com_epicgames_ue4_GameActivity_audioCapturePayload
	...
	{
		// Check for null JNIEnv or array
		if (!jenv || !shortPCMDataArray) {
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Invalid JNI environment or null array"));
			return;
		}
		...
		if (readSize <= 0) {
			UE_LOG(LogRuntimeAudioImporter, Warning, TEXT("Empty audio payload, skipping processing"));
			return;
		}
	
		TArray<int16> Int16PCMData;
		Int16PCMData.Reserve(readSize);
		
		// Get array elements with null check
		jshort* shortPCMDataArrayElements = jenv->GetShortArrayElements(shortPCMDataArray, nullptr);
		if (!shortPCMDataArrayElements) {
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Failed to get short array elements"));
			return;
		}
		...
		// Always release the JNI array elements
		jenv->ReleaseShortArrayElements(shortPCMDataArray, shortPCMDataArrayElements, 0);
		shortPCMDataArrayElements = nullptr;
	
		// Validate data before transcoding
		if (Int16PCMData.Num() == 0) {
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("No PCM data to transcode"));
			return;
		}
	
		float* FloatPCMDataPtr = nullptr;
		
		// Check the return value of TranscodeRAWData if it has one
		FRAW_RuntimeCodec::TranscodeRAWData(Int16PCMData.GetData(), Int16PCMData.Num(), FloatPCMDataPtr);
		
		if (!FloatPCMDataPtr) {
			UE_LOG(LogRuntimeAudioImporter, Error, TEXT("Transcoding failed or produced null data"));
			return;
		}
		
		// Check if callback is valid before calling
		if (AudioCaptureAndroidForCallback) {
			AudioCaptureAndroidForCallback->OnAudioCapture(FloatPCMDataPtr, readSize, 0.0, false);
		} else {
			UE_LOG(LogRuntimeAudioImporter, Warning, TEXT("Audio capture callback is null"));
		}
		
		// Safe cleanup
		if (FloatPCMDataPtr) {
			FMemory::Free(FloatPCMDataPtr);
			FloatPCMDataPtr = nullptr;
		}
	}
}
