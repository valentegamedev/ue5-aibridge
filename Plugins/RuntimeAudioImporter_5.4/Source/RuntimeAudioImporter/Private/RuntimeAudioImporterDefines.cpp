...
namespace RuntimeAudioImporter
{
	...
	bool CheckAndRequestPermissions(TArray<FString> AllRequiredPermissions)
	{
		...
		if (AllRequiredPermissions.Num() == 0)
		{
			...
			{
				TArray<FString> InternalPermissions;
				const int32 AndroidVersion = FAndroidMisc::GetAndroidBuildVersion();
				...
				{
					// Android 11-12: Legacy permissions still work but scoped
					InternalPermissions.Add(TEXT("android.permission.READ_EXTERNAL_STORAGE"));
					// WRITE_EXTERNAL_STORAGE is mostly useless here, but won't hurt
					InternalPermissions.Add(TEXT("android.permission.WRITE_EXTERNAL_STORAGE"));
				}
				else
				{
					// Android 10 and below: Full legacy permissions
					InternalPermissions.Add(TEXT("android.permission.READ_EXTERNAL_STORAGE"));
					InternalPermissions.Add(TEXT("android.permission.WRITE_EXTERNAL_STORAGE"));
				}
