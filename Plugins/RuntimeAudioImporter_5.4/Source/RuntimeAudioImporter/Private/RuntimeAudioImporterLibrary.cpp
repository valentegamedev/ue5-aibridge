...
void URuntimeAudioImporterLibrary::EncodePCMToOpus(const TArray<float> InPCM)
{
	...
	TArray<FBaseRuntimeCodec*> RuntimeCodecs = CodecFactory.GetCodecs(OpusAudioFormat);
...
bool URuntimeAudioImporterLibrary::ResampleAndMixChannelsInDecodedInfo(FDecodedAudioStruct& DecodedAudioInfo, uint32 NewSampleRate, uint32 NewNumOfChannels)
{
	...
	if (NewNumOfChannels != DecodedAudioInfo.SoundWaveBasicInfo.NumOfChannels)
	{
		...
		UE_LOG(LogRuntimeAudioImporter, Verbose, TEXT("Audio data has been mixed to the desired number of channels '%d'"), NewNumOfChannels);
	}
